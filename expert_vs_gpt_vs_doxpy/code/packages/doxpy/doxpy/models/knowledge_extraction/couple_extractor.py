...
class CoupleExtractor(CE):
	...
	@staticmethod
	def get_core_triplets(core_concept, other_core_concept, core_set, avoid_jumps=False): # can be one per core in core_set
		...
		if subj_core == core:
			subj_dict_iter = concept_dict_iter
			obj_dict_iter = other_concept_dict_iter
		else:
			obj_dict_iter = concept_dict_iter
			subj_dict_iter = other_concept_dict_iter

		triplet_list = []
		for subj_dict in subj_dict_iter:
			for obj_dict in obj_dict_iter:
				subj_span, obj_span = list(subj_dict['concept']['span']), list(obj_dict['concept']['span'])
				this_predicate_dict = dict(predicate_dict) # Shallow copy! IMPORTANT: this shallow copy prevents memory exhaustion, but it also implies that any change to these dictionaries is going to be shared. Perform a deep copy to avoid this.
				this_predicate_dict['source_text'] = get_concept_dict(sorted(predicate_span + subj_span + obj_span, key=lambda x: x.idx))['text']
				this_predicate_dict['source'] = subj_dict['source']
				
				this_verb_dict = dict(verb_dict) # Shallow copy! IMPORTANT: this shallow copy prevents memory exhaustion, but it also implies that any change to these dictionaries is going to be shared. Perform a deep copy to avoid this.
				if verb_span:
					this_verb_dict['source_text'] = get_concept_dict(sorted(verb_span + subj_span + obj_span, key=lambda x: x.idx))['text']
				this_predicate_dict['verb'] = this_verb_dict
				
				# #### free memory
				# CE.clean_concepts_from_tokens(subj_dict, remove_source=True)
				# CE.clean_concepts_from_tokens(obj_dict, remove_source=True)
				# ################
				triplet_list.append((subj_dict, this_predicate_dict, obj_dict))
		return triplet_list
	...
	def get_triplet_list_by_concept_list(concept_list, avoid_jumps=False):
		...
		# triplet_iter = flatten((
		# 	CoupleExtractor.get_core_triplets(core_concept, other_core_concept, core_set, avoid_jumps=avoid_jumps)
