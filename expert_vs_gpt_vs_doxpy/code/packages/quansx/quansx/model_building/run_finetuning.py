...
from quansx.model_building.data_collator import T2TDataCollator
from quansx.utils.transformers_lib import freeze_embeds, assert_not_all_frozen
...
class ModelArguments:
    ...
    label_smoothing: Optional[float] = field(
        default=0,
        metadata={"help": "label smoothing rate, set to > 0 if you want to enable label smoothing"}
    )
...
def main(args_file=None):
    ...
    tokenizer = tokenizer_cls.from_pretrained(
        os.path.join(data_args.dataset_path, f"tokenizer_{data_args.task}_{model_args.model_type}_{data_args.data}"),
    ...
    train_file_name = f"train_data_{data_args.task}_{model_args.model_type}_{data_args.data}.pt"
    train_path = os.path.join(data_args.dataset_path, train_file_name)
    train_dataset = torch.load(train_path) if training_args.do_train else None

    valid_file_name = f"valid_data_{data_args.task}_{model_args.model_type}_{data_args.data}.pt"
    valid_path = os.path.join(data_args.dataset_path, valid_file_name)
    valid_dataset = torch.load(valid_path) if training_args.do_eval else None
